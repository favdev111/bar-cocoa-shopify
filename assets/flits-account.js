        ...
        flitsAppJquery.ajax({
            ...
            success: function (res) {
                if (res.status) {
                  ...
                  var error_in = Object.keys(response.errors)[0];
                  var error =  error_in + " " + response.errors[error_in];           
                  if(response.errors.phone[0] == "has already been taken"){
                    flitsAppJquery("#flits-page-profile").toast(window.flitsApp.multilang.phone_has_already_been_taken +" "+ window.flitsApp.multilang.try_to_add_other_number);
                  }else{
                    flitsAppJquery("#flits-page-profile").toast(error +" "+ window.flitsApp.multilang.or_enter_country_code);
                  }
