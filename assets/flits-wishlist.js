...
//       item.addEventListener('click', function (e) {
        ...
        switch (page) {
          ...
          case "collection-page":
            ...
            wsl_btn.getElementsByClassName("flits-remove-from-wsl-btn")[0].style.display = "inline-flex";
...
//     })
   ...
   this.remove_product = function (item) {
    if(!item){
	return false;
    }	  
    var that = this;
...
//       item.addEventListener('click', function (e) {
        ...
        switch (page) {
          ...
          case "collection-page":
            ...
            wsl_btn.getElementsByClassName("flits-add-to-wsl-btn")[0].style.display = "inline-flex";
            if (wsl_btn.parentNode.getElementsByClassName('flits-wsl-product-count')[0]) {
              count = parseInt(wsl_btn.parentNode.getElementsByClassName('flits-wsl-product-count')[0].innerHTML);
              wsl_product_count = parseInt(count) - 1;
  ...
  this.flits_wsl_guest_login = function () {
    ...
    for (var i = 0; i < flits_handel_array.length; i++) {
      ...
      if (s.length > 0)
      {
        ...
        s[0].parentElement.querySelectorAll('.flits-remove-from-wsl-btn')[0].style.display = 'inline-flex';
      }
    }
  };
...
//   this.remove_product();
  ...
  document.addEventListener("click", function(event){
    if(event.target.classList.contains("flits-remove-from-wsl-btn")){
      that.remove_product(event.target);
    }
    else if(event.target.classList.contains("flits-add-to-wsl-btn")){
      that.add_product(event.target); 	
    }
  });
...
//   var addtocart_selectors = ["button[name='add']"];
  var addtocart_selectors = ["#atc-add-to-cart","#atc-add-to-cart-m"];
    ...
    if (typeof els == "object" && els) {
      for (var i = 0; i < els.length; i++) {
        ...
        if (is_wishlist_automatic == 1) {
          var n = el.parentElement.children.length;
          el.after(document.getElementById('flits-wishlist-automatic-code').cloneNode(true).children[0], el.parentElement.children[n-1])
        }
      }
    }
  })
